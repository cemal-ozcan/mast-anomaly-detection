...
**Faz 8 — Pilot/Demo** (devam ediyor) — Iter 8.1 (demo orkestrasyon) ✅ DONE (2026-06-03); **Iter 8.2 (dashboard görsel zenginleştirme) ✅ DONE (2026-06-14)**; **Iter 8.3 (arıza kapsamı: F sıcaklık aşımı + gerçekçi termal model + E sensör arızası/`sensor_out_of_range` kuralı + 6-cihaz demo) ✅ DONE (2026-06-15)**; **Iter 8.4 (skor standardizasyonu: band-pozisyon `clamp01((q−warn)/(trip−warn))` tüm dedektörlerde + füzyon temsilci-skoru dürüstlüğü + sensör-sağlığı skoru 1.0) ✅ DONE (2026-06-18)**; **Iter 8.5 (uyarı yaşam döngüsü reconciliation: in-memory `active` dict → DB-tek-hakikat + per-poll reconciliation [migration 004 `rule_set` fingerprint] + P2 manuel resolve kaldırma [yalnız ack; çözümü detector sahiplenir]; restart-orphan + manuel-resolve divergence inşaat gereği çözüldü, eskalasyon korundu) ✅ DONE (2026-06-18)**; **Iter 8.6 ("yaşayan uyarı": uyarı kimliği fingerprint→cihaz-seviyesi tek olay + in-place UPDATE [eskalasyon tek satırda + skor/severity refresh, donma yok] + severity band skorundan türetme [`severity_bands` config, validity-rule muaf] + ack sonrası band-yukarı re-activate [guard'lı]) ✅ DONE (2026-06-19)**; **Iter 8.7 (deadband hysteresis: cihaz firing değilse bile arıza N ardışık temiz poll yoksa resolve etme [DB-backed `clean_streak` sayacı, migration 005, config `deadband_clean_polls=3`]; firing sayacı sıfırlar → salınan arıza tek satır kalır, anti-flap) ✅ DONE (2026-06-19)**; **Iter 8.8 (veri-kalitesi ekseni: sensör-sağlığı uyarıları dashboard'da ayrı "Veri Kalitesi" panelinde; saf `split_alerts_by_axis` + `Alert.rule_set` + `VALIDITY_RULES` tek-kaynak `base.py` + `fleet.py` rank dedup; gating gerçek-donanıma ertelendi) ✅ DONE (2026-06-19).** **→ FAZ 8 KAPANDI: POC sağlamlaştırma planı (8.3-8.8) tamamlandı.** Faz 1-5 + Faz 7 ✅ DONE. **Faz 6 — ML dedektörler (Isolation Forest) ATLANDI / Faz 9+ stretch'e ertelendi** (kullanıcı kararı 2026-06-03: simülatör yalnız A/B/C üretiyor, ikisi de mevcut iki katmanca yakalanıyor → ML'in sentetik veride marjinal tespit değeri düşük + en yüksek karmaşıklık).

**POC sağlamlaştırma planı (kullanıcı, 2026-06-14):** Hedef = mast firmasını gerçek cihaz tahsisine ikna edecek POC. 4 iyileştirme iterasyona bölündü: **Iter 8.3 ✅ (arıza kapsamı F+E — D atıldı: tespiti A ile örtüşür, "şişirilmiş sayı yerine net tespit")**, **Iter 8.4 ✅ (skor standardizasyonu — band-pozisyon; ISO 13374/20816 + ISA-18.2 çift-araştırma temelli)**, **Iter 8.5 ✅ (uyarı yaşam döngüsü reconciliation)** + **Iter 8.6 ✅ ("yaşayan uyarı": eskalasyon update-in-place + skor/severity refresh + severity-banttan-türetme)**; **Iter 8.7 ✅ (deadband hysteresis — anti-flap; (3) yapıldı)** + **Iter 8.8 ✅ (veri-kalitesi ekseni sunum ayrımı — dashboard ayrı panel + `fleet.py` rank dedup; (5)'in sunum kısmı; **gating gerçek-donanım fazına ertelendi — sentetik veride görünmez**)**. **→ POC sağlamlaştırma planı TAMAMLANDI (8.3-8.8); Faz 8 kapandı.** Web araştırması (2026-06-14): 6 parametremiz (motor akım/voltaj, hidrolik basınç, sıcaklık, pozisyon, titreşim) gerçek teleskopik mast/bom durum-izleme standardıyla örtüşüyor; gerçek sistemlerin ek ölçtüğü yük/eğim/yağ-durumu POC kapsamı dışı (gerçek-donanım fazında entegre). ML'e GİRİLMEYECEK (kullanıcı net).
...
### Iterasyon 8.7 (Deadband Hysteresis — Anti-Flap) — Tamamlandı (2026-06-19)

8.6'dan ertelenen (3) hysteresis parçası. **Sorun:** cihaz "firing değil" olunca `_detect_once` açık uyarıyı HEMEN resolve ediyordu → değer eşik civarında salınınca resolve↔reopen churn (8.1 "kozmetik flicker"). **Çözüm:** deadband — arıza **N ardışık temiz poll** yok olmadan resolve etme. Brainstorming (kapsam: (3) alındı, (5) → 8.8; mekanizma: deadband vs Schmitt → deadband seçildi, kural kalibrasyonu gerekmez) → spec → self-review (YAGNI çekincesi dürüstçe belgelendi: 120s pencere + 8.6 in-place zaten flapping'i büyük ölçüde yumuşatıyor → bu ucuz sigorta) → bağımsız plan-review (APPROVE-WITH-FIXES, 1 BLOCKER: integration schema_version count → işlendi) → subagent-driven 5 task (controller-inline + reviewer hibrit) → final whole-branch review (APPROVE). **Migration 005** `anomalies.clean_streak INTEGER NOT NULL DEFAULT 0` (DB-backed sayaç → 8.5/8.6 "DB tek hakikat + stateless detector + level-triggered" korunur, restart-safe; **yorumda noktalı virgül YOK** — migrator naif `;` split). `Alert.clean_streak: int = 0` (default'lu → mevcut constructor'lar kırılmaz) + `_row_to_alert` taşır. Repository: `set_clean_streak(alert_id, streak)` + `update_alert` artık `clean_streak=0` (firing reset). **`_detect_once` clean+open dalı** (YALNIZ bu dal genişler; 8.6 dört-kadran iskelet korunur): `new_streak = primary.clean_streak+1`; `>= deadband_clean_polls` → `resolve_open_alerts` else `set_clean_streak`. Config `DetectorConfig.deadband_clean_polls: int = 3` (default'lu; example+demo YAML). **Değişmez:** `Anomaly`/`fuse_anomalies`/`Detector` ABC/`band_position_score`/severity-banttan+re-activate+in-place (8.6)/gözlem modu/ACK yaşam döngüsü. **402→406 test** + mypy strict (104 dosya) + ruff temiz. **Canlı 6-cihaz smoke:** cihaz başına tek satır boyunca **churn yok** (resolved 0→1, flap yığını yok); device_003 arıza temizlenince clean_streak tırmandı (cs=2 donmuş gözlendi) → 3. poll'da resolve (anlık değil, deadband); 8.6 korundu (005 skoru 0.48→0.58→0.68 canlı tırmandı, in-place tek satır). **Bilinen sınır (spec § 4, kabul):** idempotency — aynı poll iki kez koşarsa streak +2 ("yaklaşık N poll" semantiği; runtime'da poll'lar serileştirilir). **D atıldı zaten; (5) veri-kalitesi ekseni + `fleet.py` rank dedup → Iter 8.8 (opsiyonel).** Spec/plan: `docs/specs/2026-06-19-faz8-iter8-7-deadband-hysteresis-design.md` + `docs/plans/2026-06-19-faz8-iter8-7-deadband-hysteresis.md`.

### Iterasyon 8.8 (Veri-Kalitesi Ekseni — Sensör-Sağlığı Sunum Ayrımı) — Tamamlandı (2026-06-19)

POC sağlamlaştırmanın SON iterasyonu (8.6/8.7'den ertelenen (5)'in sunum kısmı). **Sorun:** sensör-bütünlüğü uyarıları (`sensor_out_of_range`/`sensor_frozen` = `VALIDITY_RULES`) kestirimci arızalarla tek dashboard tablosunda karışıktı. **Çözüm:** SUNUM-tarafı eksen ayrımı — dashboard'da iki ayrı panel ("🚨 Arıza Uyarıları" + "🔌 Veri Kalitesi / Sensör Sağlığı"). Brainstorming (kapsam: A=sunum alındı, **B=gating ertelendi** — sentetik veride görünmez [`mast_position` kestirimci kuralca okunmuyor, `sensor_frozen` simülatörde üretilmiyor], görünür kılmak yeni "donmuş sensör" senaryosu gerektirir → gerçek-donanım fazı) → spec → bağımsız plan-review (APPROVE-WITH-FIXES, 1 MINOR test-import) → subagent-driven 5 task (controller-inline + reviewer hibrit) → final review (APPROVE). **`VALIDITY_RULES` tek-kaynak `service.py`→`base.py`'ye** (saf leaf; detector + dashboard paylaşır). **`Alert.rule_set: str | None = None`** okuma alanı (+`_row_to_alert`; kolon migration 004'ten var, **migration YOK**). **Saf `transform.py`:** `is_data_quality_alert(alert)` (rule_set'teki TÜM kurallar `VALIDITY_RULES`'taysa True; None/boş/karışık→False) + `split_alerts_by_axis(alerts)->(faults, data_quality)`. **`app.py` `_render_overview`:** tek tablo → `split_alerts_by_axis` + `_render_alert_table` helper x2 (her panel kendi boş-durumu; filtre/özetleme split'ten ÖNCE; sorgu bütçesi değişmedi). **Cleanup:** `fleet.py` dup `_SEVERITY_RANK` → `from detectors.fusion import SEVERITY_RANK`. **Değişmez:** `Anomaly`/`fuse_anomalies`/`Detector` ABC/`_detect_once` reconciliation+deadband(8.7)+severity-banttan(8.6)/füzyon/gözlem modu/migration şeması. **414 test** + mypy strict (136 dosya) + ruff temiz. **Canlı 6-cihaz smoke:** gerçek `split_alerts_by_axis` ile 006 (`sensor_out_of_range`) → Veri Kalitesi paneli, 002/003/004 (kestirimci) → Arıza paneli. **Bilinen sınır (kabul):** aynı cihazda çakışan fault+bad-sensor (demoda yok) füzyonla tek satırda → kestirimci panele düşer; tam ayrım gating ile gerçek-donanımda. Spec/plan: `docs/specs/2026-06-19-faz8-iter8-8-data-quality-axis-design.md` + `docs/plans/2026-06-19-faz8-iter8-8-data-quality-axis.md`.

## Faz 8 Closure (2026-06-19)

Faz 8 = Pilot/Demo + POC sağlamlaştırma. **8 iterasyon:** 8.1 demo orkestrasyon → 8.2 dashboard görsel → 8.3 arıza kapsamı (F+E) → 8.4 skor standardizasyonu (band-pozisyon) → 8.5 uyarı yaşam döngüsü reconciliation → 8.6 yaşayan uyarı (in-place + severity-banttan) → 8.7 deadband hysteresis → 8.8 veri-kalitesi ekseni. **Kabul kriterleri karşılandı:** tek komutla 15-dk demo (`demo_up.sh`), 4 kestirimci arıza (A/B/C/F) + 1 veri-kalitesi (E), komuta-merkezi dashboard (KPI + filo kartları + iki-eksen uyarı panelleri), olgun alarm yaşam döngüsü (ack + auto-resolve + eskalasyon in-place + deadband + re-activate), ISO/ISA-temelli standart skorlama. 414 test, mypy strict + ruff temiz. **Kapsam dışı / gerçek-donanım fazına ertelenen:** ML dedektörler (Faz 6 atlandı), gating (sensör-geçersizken kestirimci askıya alma), Katman 2/Faz 9+ (Docker/TimescaleDB/Grafana/FastAPI/LSTM — kullanıcı istemiyor), gerçek donanım sensörleri (yük/eğim/yağ). **Kapsam içinde iş kalmadı → sıradaki büyük adım gerçek cihaz tahsisine bağlı.**

## Dashboard Yeniden Tasarımı — Sunum Hazırlığı (2026-06-21)

Yönetim kurulu başkanı + IT lead sunumu için dashboard'un baştan elden geçirilmesi (`dashboard-redesign-clean-corporate` dalı; brainstorming → spec → plan → subagent-driven). Hedef: sade, kendini-açıklayan, düz Türkçe (kod kimliği yok), arkadaki çok-katmanlı işi hak eden "operasyon merkezi" görünümü. Streamlit-native (React'e geçilmedi). **3 sayfa:** (1) **Operasyon Merkezi** (açılış: sağlık halkası + KPI bandı + duruma-göre-sıralı filo kartları [inline-SVG sparkline] + 24s olay-akışı timeline'ı + iki-eksen "Açık Uyarılar" [Arıza / Veri Kalitesi]); (2) **Cihaz Detayı** (sensör başına tam-satır: solda durum paneli [düz-dil "ne sorun/ne anlama geliyor" + katman chip'leri "İZLEYEN/YAKALAYAN" + ISO 20816 alarm-seviyesi gauge], sağda hibrit-ölçekli radar grafik [yeşil/sarı/kırmızı bölge + eşik çizgileri]); (3) **Canlı Veri Akışı** (cihaz başına ham SQLite telemetri tablosu, salt-okuma).

**Saf-leaf mimari korundu:** `src/dashboard/{labels,thresholds,detection,overview,fleet,charts,transform,styles}.py` streamlit/DB import ETMEZ (birim-testli saf HTML/SVG/veri builder'ları); `app.py` tek Streamlit wiring katmanı. **Dashboard TAMAMEN salt-okuma** (ack/manuel-resolve kontrolü kaldırıldı — "özellik vitrini, operasyonel araç değil"; gözlem modu mutlak). **Enstrüman-sınıfı görünüm** (frontend-design): IBM Plex Sans/Mono + grafit palet, emoji/AI-mavisi yok, renk=anlam. Storage'a salt-okuma read'ler eklendi (`earliest_telemetry_timestamp`, `count_anomalies_since`, `fetch_latest_readings`, `fetch_recent_for_device`).

**Demo kalıcı arızalar:** `config/devices.demo.yaml` arıza `duration_s`→86400 (kalıcı). Sebep: zaman-kutulu arızalar 3-31 dk'da bitip dönmüyordu → demo uzun açık kalınca anomali görünmüyordu. Kalıcı yapınca demo ne zaman başlatılırsa ~1-2 dk sonra anomaliler belirir ve kapanmaz → sunum zamanlamasına bağımsız. (`test_demo_configs`: kısa-arıza sözleşmesi → kalıcı-arıza.)

**KRİTİK render dersi (kalıcı):** Streamlit'in tarayıcı **vega-embed**'i **katmanlı (bant'lı) Altair grafiğinde x-zamanlı katmanları (çizgi + bölge) sessizce düşürüyor** → bant'lı sensör grafikleri BOŞ çıkıyordu (tek-katmanlı grafikler düzgün; spec doğru — vl-convert ile izole render kusursuz). **İlk iki hipotez (adlandırılmış dataset, açık x-domain) YANLIŞTI; gerçek Vega motoruyla (vl-convert) PNG render ederek teşhis kanıtlandı.** **Çözüm:** detay grafiklerini SUNUCUDA PNG'ye render et (`_chart_to_png` → `vl_convert.vegalite_to_png`) + `st.image` → vega-embed'i tümden bypass (canlı izlemede zoom gereksiz; gözlem modu korunur). Yeni bağımlılık: `vl-convert-python==1.9.0` (kendi-yeten Rust binary; pyproject mypy override). **Ders: tarayıcı-render hatalarını TAHMİNLE değil gerçek motorla render ederek teşhis et.**

**Saat-dilimi:** chart frame'leri tz-naive UTC (`tz_localize(None)`; telemetri çizgisi + uyarı overlay'i AYNI işlem, yoksa kayar). Sonuç: grafik ekseni üstteki saat/veri-akışıyla tutarlı (hepsi UTC; yerel-saate çevirme kullanıcı kararıyla ertelendi — fonksiyonel değil).

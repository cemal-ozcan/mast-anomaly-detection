...
**Faz 8 — Pilot/Demo** (devam ediyor) — Iter 8.1 (demo orkestrasyon) ✅ DONE (2026-06-03); **Iter 8.2 (dashboard görsel zenginleştirme) ✅ DONE (2026-06-14)**; **Iter 8.3 (arıza kapsamı: F sıcaklık aşımı + gerçekçi termal model + E sensör arızası/`sensor_out_of_range` kuralı + 6-cihaz demo) ✅ DONE (2026-06-15).** Faz 1-5 + Faz 7 ✅ DONE. **Faz 6 — ML dedektörler (Isolation Forest) ATLANDI / Faz 9+ stretch'e ertelendi** (kullanıcı kararı 2026-06-03: simülatör yalnız A/B/C üretiyor, ikisi de mevcut iki katmanca yakalanıyor → ML'in sentetik veride marjinal tespit değeri düşük + en yüksek karmaşıklık).

**POC sağlamlaştırma planı (kullanıcı, 2026-06-14):** Hedef = mast firmasını gerçek cihaz tahsisine ikna edecek POC. 4 iyileştirme iterasyona bölündü: **Iter 8.3 ✅ (arıza kapsamı F+E — D atıldı: tespiti A ile örtüşür, "şişirilmiş sayı yerine net tespit")**, sıradaki **Iter 8.4 (skor standardizasyonu — fusion max(score) dürüstlüğü)** + **Iter 8.5 (uyarı yaşam döngüsü kenar durumları: restart orphan, süregelen-arıza manuel-resolve, eskalasyon)**. Web araştırması (2026-06-14): 6 parametremiz (motor akım/voltaj, hidrolik basınç, sıcaklık, pozisyon, titreşim) gerçek teleskopik mast/bom durum-izleme standardıyla örtüşüyor; gerçek sistemlerin ek ölçtüğü yük/eğim/yağ-durumu POC kapsamı dışı (gerçek-donanım fazında entegre). ML'e GİRİLMEYECEK (kullanıcı net).

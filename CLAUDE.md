...
**Faz 8 — Pilot/Demo** (devam ediyor) — Iter 8.1 (demo orkestrasyon) ✅ DONE (2026-06-03); **Iter 8.2 (dashboard görsel zenginleştirme) ✅ DONE (2026-06-14)**; **Iter 8.3 (arıza kapsamı: F sıcaklık aşımı + gerçekçi termal model + E sensör arızası/`sensor_out_of_range` kuralı + 6-cihaz demo) ✅ DONE (2026-06-15)**; **Iter 8.4 (skor standardizasyonu: band-pozisyon `clamp01((q−warn)/(trip−warn))` tüm dedektörlerde + füzyon temsilci-skoru dürüstlüğü + sensör-sağlığı skoru 1.0) ✅ DONE (2026-06-18).** Faz 1-5 + Faz 7 ✅ DONE. **Faz 6 — ML dedektörler (Isolation Forest) ATLANDI / Faz 9+ stretch'e ertelendi** (kullanıcı kararı 2026-06-03: simülatör yalnız A/B/C üretiyor, ikisi de mevcut iki katmanca yakalanıyor → ML'in sentetik veride marjinal tespit değeri düşük + en yüksek karmaşıklık).

...
**Faz 7 ✅ DONE. Faz 8 ✅ DONE (2026-06-19)** (Faz 6 ML stretch'e ertelendi). **Kapsam içi iş kalmadı — sıradaki büyük adım gerçek cihaz tahsisine bağlı.**

---

## Faz 8 — Pilot ve Demo ✅ TAMAMLANDI
...
- Kod kalitesi review için hazır — ✅ (414 test, mypy strict + ruff temiz, CI: GitHub Actions ruff+mypy+pytest)
...
- ✅ Iter 8.4 — Skor standardizasyonu (band-pozisyon): `band_position_score(q,warn,trip)=clamp01((q−warn)/(trip−warn))` tüm dedektörlerde (0=alarmı yeni geçti, 1=kritik); ISO 13374/20816 + ISA-18.2 çift web-araştırması temelli. 5 eşik kuralı config-driven `trip_*` (gerçek sim çıktısıyla kalibre: temp 130/current 11/vibration 0.50/hydraulic 6.0/voltage 5.0) + istatistik standart çapa (3σ→6σ, IQR→Tukey 3·IQR). Füzyon dürüstlüğü (`score=top.score`, ödünç max kalktı) + sensör-sağlığı skoru 1.0 (ikili validity). 380 test + mypy strict (135 dosya) + ruff temiz; canlı 6-cihaz smoke (001 0 FP; 004 std→0.578; 002 `fused(6)` high temsilci-skoru 0.021 ödünç-değil; 006 1.0; istatistik satürasyon dürüst). D (severity-bant) + E (veri-kalitesi ekseni) → Iter 8.6. — 2026-06-18. Spec/plan: `docs/specs/2026-06-17-faz8-iter8-4-score-standardization-design.md` + `docs/plans/2026-06-17-faz8-iter8-4-score-standardization.md`.
- ✅ Iter 8.5 — Uyarı yaşam döngüsü reconciliation: in-memory `active` dict → **DB-tek-hakikat + per-poll idempotent reconciliation** (level-triggered; Claude + 2 harici AI araştırma örtüşmesi). Migration 004 `anomalies.rule_set` fingerprint (sıralı virgül-bağlı kural adları) + `fetch_open_fingerprints`. `_detect_once` reworku: cihaz-temiz→resolve-all, fingerprint-eşleşir(ack dahil)→debounce, eşleşmez→yeni satır (çözüm cihaz-seviyesi, açma fingerprint-seviyesi → **eskalasyon/limit 3 korunur**); restart=normal poll yolu → **orphan/duplikat yok**. **P2: manuel resolve kaldırıldı** (dashboard butonu + `resolve_alert` silindi; gözlem-modu → çözümü detector sahiplenir, yalnız ack). Faz 7 § 9 limit 1+2 inşaat gereği çözüldü; limit 3 korundu (update-in-place Iter 8.6). 384 test + mypy strict (135 dosya) + ruff temiz; canlı 6-cihaz smoke + restart (rule_set doluyor, auto-resolve doğru, 28dk+restart 22 satır=duplikat yok). — 2026-06-18. Spec/plan: `docs/specs/2026-06-18-faz8-iter8-5-alert-lifecycle-reconciliation-design.md` + `docs/plans/2026-06-18-faz8-iter8-5-alert-lifecycle-reconciliation.md`.
- ✅ Iter 8.6 — "Yaşayan uyarı": uyarı kimliği fingerprint→**cihaz-seviyesi tek olay** + in-place UPDATE (eskalasyon tek satırda + skor/severity refresh, donma yok) + severity band skorundan türetme (`severity_bands` config, validity-rule muaf) + ack sonrası band-yukarı **re-activate** (guard'lı). `update_alert` temsilciden türeyen TÜM alanları (rule_name/sensor dahil) tazeler (ödünç-alan tutarsızlığı yok). Migration yok. 399 test + mypy strict + ruff temiz; canlı 6-cihaz smoke (005 skor 0.03→1.0 warning→high→critical aynı satırda; ack→critical re-activate; restart orphan/duplikat yok). — 2026-06-19. Spec/plan: `docs/specs/2026-06-18-faz8-iter8-6-living-alert-design.md` + `docs/plans/2026-06-18-faz8-iter8-6-living-alert.md`.
- ✅ Iter 8.7 — Deadband hysteresis (anti-flap): cihaz "firing değil" olsa bile arıza **N ardışık temiz poll** yoksa resolve etme (DB-backed `clean_streak`, migration 005, config `deadband_clean_polls=3`); firing sayacı sıfırlar → salınan arıza tek satır kalır. 406 test + mypy strict + ruff temiz; canlı smoke (churn yok, device_003 deadband resolve). — 2026-06-19. Spec/plan: `docs/specs/2026-06-19-faz8-iter8-7-deadband-hysteresis-design.md` + `docs/plans/2026-06-19-faz8-iter8-7-deadband-hysteresis.md`.
- ✅ Iter 8.8 — Veri-kalitesi ekseni (sensör-sağlığı sunum ayrımı): sensör-bütünlüğü uyarıları (`sensor_out_of_range`/`sensor_frozen`) dashboard'da kestirimci arızalardan AYRI "Veri Kalitesi" panelinde; saf `split_alerts_by_axis`/`is_data_quality_alert` + `Alert.rule_set` + `VALIDITY_RULES` tek-kaynak `base.py` + `fleet.py` rank dedup. Gating gerçek-donanım fazına ertelendi (sentetik veride görünmez). 414 test + mypy strict + ruff temiz; canlı smoke (006→veri-kalitesi paneli, kestirimci→arıza paneli). — 2026-06-19. Spec/plan: `docs/specs/2026-06-19-faz8-iter8-8-data-quality-axis-design.md` + `docs/plans/2026-06-19-faz8-iter8-8-data-quality-axis.md`.

**Faz 8 Closure (2026-06-19):** POC sağlamlaştırma planı (8.3 arıza kapsamı → 8.4 skor std → 8.5 reconciliation → 8.6 yaşayan uyarı → 8.7 deadband → 8.8 veri-kalitesi ekseni) tamamlandı. Tek-komut 15-dk demo + 4 kestirimci arıza (A/B/C/F) + 1 veri-kalitesi (E) + komuta-merkezi dashboard (iki-eksen panel) + olgun alarm yaşam döngüsü (ack/auto-resolve/eskalasyon-in-place/deadband/re-activate) + ISO/ISA-temelli skorlama + CI. 414 test. **Kapsam içi iş kalmadı; gating + ML + Katman 2 (aşağıdaki Faz 9+) gerçek-donanım/stretch.**

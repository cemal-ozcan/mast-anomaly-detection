...
**`src/dashboard/fleet.py`** (MODIFY): saf `sensor_badge(device_alerts: list[Alert], sensor: str) -> str` ekle → o sensöre açık uyarıların **en kötü** severity'sini `ok|warning|critical` rozetine çevirir (uyarı yoksa `ok`). Mevcut severity→rozet eşlemesini ve `SEVERITY_RANK`'i yeniden kullanır (yeni eşleme icat etme). Türetim çekirdeği (`derive_fleet`/`compute_kpis`) DEĞİŞMEZ.
...
- **Gezinme:** sol menüde tek seçim — `🏠 Filo Genel Bakış` + her cihaz (`device_label`). Seçim `st.session_state["nav"]` (key'li `st.sidebar.radio`/`selectbox`). `🏠 Filo` → mevcut `_render_overview` + `_render_alert_management`. Bir cihaz → yeni `_render_device_detail(repository, device_id, window, detector_config)`.
